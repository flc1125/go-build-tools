...
func TestWorkSkip(t *testing.T) {
	lg, _ := zap.NewDevelopment()
	config := RunConfig{
		Logger:       lg,
		GoVersion:    "1.20",
		SkippedPaths: map[string]struct{}{filepath.Join("testB", "go.mod"): {}},
	}

	tmpRootDir := createTempTestDir(t, "testWork")
	require.NoError(t, os.Remove(filepath.Join(tmpRootDir, "go.work")))
	require.NoError(t, renameGoMod(tmpRootDir))
	config.RootPath = tmpRootDir

	require.NoError(t, Work(config))
	assertGoWork(t, "go 1.20\nuse .\nuse ./testA", tmpRootDir)
}
...
func assertGoWork(t *testing.T, expected, tmpRootDir string) {
	...
	assert.Empty(t, diff, "go.work mismatch (-want +got)")
}
