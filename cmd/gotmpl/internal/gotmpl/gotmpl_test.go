...
func Test(t *testing.T) {
	...
	err = Run(bodyPath, jsonData, destPath)
	require.NoError(t, err)
...
func TestReplacesExistingOutput(t *testing.T) {
	want, err := os.ReadFile("testdata/want.go")
	require.NoError(t, err)

	destPath := filepath.Join(t.TempDir(), "out.go")
	require.NoError(t, os.WriteFile(destPath, []byte("old output"), 0o600))
...
func TestMissingData(t *testing.T) {
	...
	require.NoError(t, os.WriteFile(destPath, original, 0o600))

	err := Run(bodyPath, `{ "badkey": "val" }`, destPath)

	assert.ErrorContains(t, err, "gotmpl: execution failed")
	actual, readErr := os.ReadFile(destPath) //nolint:gosec // It is a safe test temp filepath.
...
func TestNotExistingBody(t *testing.T) {
	...
	err := Run("testdata/non-exiting.go", jsonData, destPath)
