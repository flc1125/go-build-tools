...
func GetModuleSet(modSetName, versioningFilename string) (ModuleSet, error) {
	...
	if !ok {
		return ModuleSet{}, fmt.Errorf("module set %q not found in versioning file %v", modSetName, versioningFilename)
	}
	return modSet, nil
}
...
func updateGoModVersions(modFilePath ModuleFilePath, newModRefs []ModuleRef) error {
	...
	cleanPath := filepath.Clean(string(modFilePath))
	newGoModFile, err := os.ReadFile(cleanPath)
	if err != nil {
		return fmt.Errorf("error reading go.mod file %v: %w", modFilePath, err)
	}
	...
	if err := os.WriteFile(cleanPath, newGoModFile, 0o600); err != nil { //nolint:gosec // The CLI intentionally updates configured go.mod paths.
