...
func Run(ctx context.Context, myVersioningFile string, otherVersioningFile string, otherRepoRoot string, otherModuleSetNames []string, otherVersionCommit string, allModuleSets bool, skipModTidy bool) error {
	...
	for _, moduleSetName := range otherModuleSetNames {
		...
		if err = s.updateAllGoModFiles(ctx); err != nil {
...
func (s sync) parseVersionInfo(ctx context.Context, pkg, tag string) (string, error) {
	requestURL := fmt.Sprintf("https://proxy.golang.org/%s/@v/%s.info", pkg, tag)
	req, err := http.NewRequestWithContext(ctx, http.MethodGet, requestURL, nil)
	if err != nil {
		return "", fmt.Errorf("failed to create request for package %q at version %q: %w", pkg, tag, err)
	}
...
func (s sync) updateAllGoModFiles(ctx context.Context) error {
	...
	for _, mod := range s.OtherModuleSet.Modules {
		g.Go(func() error {
			...
			if s.OtherModuleVersionCommit != "" {
				version, err := s.parseVersionInfo(ctx, string(mod), s.OtherModuleVersionCommit)
				if err != nil {
					return err
				}
				ver = version
